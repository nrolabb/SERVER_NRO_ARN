...
public class DataGame {
    ...
    public static void sendIcon(MySession session, int id) {
        Message msg;
        try {
            final byte[] icon = FileIO.readFile(getIconPath(session, id));
    ...
    private static String getIconPath(MySession session, int id) {
        int hero1Index = id - nro.models.consts.ConstPlayer.HERO1_BIEN_HINH_ICON_START + 1;
        if (hero1Index >= 1 && hero1Index <= nro.models.consts.ConstPlayer.HERO1_BIEN_HINH_FRAME_COUNT) {
            return String.format("data/icon/x%d/Hero1_%02d.png", session.zoomLevel, hero1Index);
        }
        return "data/icon/x" + session.zoomLevel + "/" + id + ".png";
    }
