...
import nro.models.network.ClientVerifier;
...
public class Controller implements IMessageHandler {
        ...
        try {
            ...
            switch (cmd) {
                ...
                case -74: {
                    ...
                    mySession.dataRequestCount++;

                    if (now - mySession.firstDataRequestTime < 5000) {
                        if (mySession.dataRequestCount > 100) {
                            Logger.warning("[RATELIMIT] " + ip + " vượt giới hạn tải dữ liệu → kick");
                            _session.disconnect();
                            break;
                        }
                    } else {
    ...
    private void sendThongBaoServer(Player player) {
        Service.gI().sendThongBaoFromAdmin(player, "Code: tanthu"
                // + "\nSự kiện Goku Day."
                // + "\nĐua TOP nhận quà cực khủng."
                // + "\nTích điềm đổi quà."
                + "\nServer Không Buff Bẩn, Không Người Nhà");
    }
