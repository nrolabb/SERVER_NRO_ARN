...
public class DanhBongSaoPhaLe {
    ...
    private static final int PORATA_VIP_MATERIAL_ID = 14;
    private static final int[] PORATA_VIP_OPTION_IDS = {50, 77, 103, 5};

    public static void showInfoCombine(Player player) {
        if (player.combineNew.itemsCombine.size() == 2) {
            Item porataVip = getPorataVip(player);
            Item material = getPorataVipMaterial(player);
            if (porataVip != null && material != null) {
                player.combineNew.goldCombine = 0;
                player.combineNew.gemCombine = 0;
                player.combineNew.ratioCombine = RATIO_NANG_CAP;

                String npcSay = "|2|Nâng cấp Porata Vip\n";
                npcSay += "|2|Tỉ lệ thành công: " + player.combineNew.ratioCombine + "%\n";
                npcSay += "|2|Cần 1 " + material.template.name + "\n";
                npcSay += "|2|Thành công sẽ nhận chỉ số từ 1-10%\n";
                npcSay += "|7|Có thể nâng cấp nhiều lần\n";
    ...
    public static void danhBongSaoPhaLe(Player player) {
        if (player.combineNew.itemsCombine.size() == 2) {
            ...
            Item material = getPorataVipMaterial(player);
            if (porataVip != null && material != null) {
                upgradePorataVip(player, porataVip, material);
                return;
            }
    ...
    private static Item getPorataVip(Player player) {
        for (Item item : player.combineNew.itemsCombine) {
            if (item != null && item.isNotNullItem() && item.template.type == 36) {
                return item;
            }
        }
        return null;
    ...
    private static Item getPorataVipMaterial(Player player) {
        for (Item item : player.combineNew.itemsCombine) {
            if (item != null && item.isNotNullItem() && item.template.id == PORATA_VIP_MATERIAL_ID) {
                return item;
            }
        }
        return null;
    ...
    private static void upgradePorataVip(Player player, Item porataVip, Item material) {
        InventoryService.gI().subQuantityItemsBag(player, material, 1);

        for (int optionId : PORATA_VIP_OPTION_IDS) {
            removeOption(porataVip, optionId);
            porataVip.itemOptions.add(new Item.ItemOption(optionId, Util.nextInt(1, 10)));
        }

        InventoryService.gI().sendItemBags(player);
        CombineService.gI().sendEffectSuccessCombine(player);
        Service.gI().sendThongBao(player, "Nâng cấp Porata Vip thành công.");
        CombineService.gI().reOpenItemCombine(player);
    }

    private static void removeOption(Item item, int optionId) {
