...
private void addGold(Player pl, long amount) {
    ...
    if (currentGold >= maxGold) {
        Service.gI().sendThongBao(pl, "Vàng của bạn đã đạt giới hạn tối đa!");
        return;
    }

    // 3. Thực hiện cộng và ép trần
    if (currentGold + amount > maxGold || currentGold + amount < 0) { 
        // (Kiểm tra < 0 để phòng trường hợp tràn số long nếu amount quá lớn)
        pl.inventory.gold = maxGold;
    } else {
        pl.inventory.gold += amount;
    }

    // 4. Cập nhật lên Client và thông báo
    PlayerService.gI().sendInfoHpMpMoney(pl);
    Service.gI().sendThongBao(pl, "Bạn nhận được " + Util.numberToMoney(amount) + " vàng");
}
