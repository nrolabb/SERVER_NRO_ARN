...
import nro.models.Farm.CloudGarden;
import nro.models.Farm.CropType;
import nro.models.Farm.FarmPlot;
...
public class FarmService {
    ...
    private void sendOpenPlotUnlockMenu(Player player, FarmPlot plot) {
        try {
            ...
            msg.writer().writeShort(ConstNpc.CON_MEO); // NPC ID placeholder
    ...
    public void fertilizePlot(Player player, int plotId, short fertilizerItemId) {
        ...
        // Nếu phân bón đã đẩy cây lên STAGE_MATURE và vượt qua thời gian chín muồi,
        // cắt phần thời gian dư thừa để cây không bị héo và thời gian về 0
        if (plot.getCurrentStage() == ConstFarm.STAGE_MATURE) {
            CropTemplate crop = CropTemplate.getById(plot.getCropType());
            if (crop != null) {
                long matureTime = crop.getStageTimeMs(ConstFarm.STAGE_MATURE);
                long timeSinceMature = System.currentTimeMillis() - plot.getLastStageChangeTime();
                
                if (timeSinceMature >= matureTime) {
                    plot.setLastStageChangeTime(System.currentTimeMillis() - matureTime);
                    plot.setMatureReadyTime(System.currentTimeMillis());
                }
            }
        }
    ...
    public void sendGardenUpdate(Player player) {
        ...
        // Đảm bảo số lượng ô đã mở khóa không nhỏ hơn mặc định
        if (player.cloudGarden.getUnlockedPlots() < ConstFarm.INITIAL_PLOTS) {
            player.cloudGarden.setUnlockedPlots(ConstFarm.INITIAL_PLOTS);
        }
        
        // Auto upgrade for old data: Luôn đảm bảo các ô từ 0 đến unlockedPlots - 1 phải được unlocked=true
        for (int i = 0; i < player.cloudGarden.getUnlockedPlots(); i++) {
