...
public void checkDoneTaskUseItem(Player player, Item item) {
    if (player.isPl() && item.isNotNullItem()) {
        switch (item.template.id) {
            case 728 -> {
                doneTask(player, ConstTask.TASK_17_6);
            }
        }
    }
}
    ...
    private int transformNpcId(Player player, int id) {
        if (id == ConstTask.NPC_NHA) {
            ...
            return player.gender == ConstPlayer.TRAI_DAT
                    ...
                    else if (id == ConstNpc.BO_MONG)
    return ConstNpc.BO_MONG;
        return id;
    }
...
//         quantity = Util.nextInt(50, 100);
//         item77 = ItemService.gI().createNewItem((short) 77);
//         item77.quantity = quantity;
//         InventoryService.gI().addItemBag(player, item77);
//         break;
    ...
    if (player.playerTask.sideTask.template != null) {
        if (player.playerTask.sideTask.isDone()) {
            ...
            switch (player.playerTask.sideTask.level) {
                ...
                case ConstTask.NORMAL:
                    goldReward = ConstTask.GOLD_NORMAL;
                    quantity = Util.nextInt(4, 6);
                    pointReward = 7;
                    break;

                case ConstTask.HARD:
                    goldReward = ConstTask.GOLD_HARD;
                    quantity = Util.nextInt(10, 12);
                    pointReward = 10;
                    break;

                default:
                    Service.gI().sendThongBao(player, "Nhiệm vụ không hợp lệ.");
                    return;
            }
            ...
            if (InventoryService.gI().getCountEmptyBag(player) > 0) {
                ...
                ngocXanh.quantity = quantity;
                InventoryService.gI().addItemBag(player, ngocXanh);

                InventoryService.gI().sendItemBags(player);

                player.playerTask.sideTask.point += pointReward;
